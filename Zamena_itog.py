...
def input_values(message): # Функция для ввода данных
    values = []
    while True:
...
def print_values(values, label): # Функция для вывода данных о r(t) и S(t)
    output_text = f"\n\n{label} по периодам:\n"
...
def conditional_optimization(Rt, St, P): # Условная оптимизация
    T = len(Rt) - 1
...
def unconditional_optimization(cond_opt): # Безусловная оптимизация
